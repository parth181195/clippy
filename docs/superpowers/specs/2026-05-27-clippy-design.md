...
| D5 | **Pin and Favorite are two orthogonal axes** (from design handoff) | Pin = position discipline (accent top stripe, always-first ordering, `P` key); Favorite = retention discipline (star icon, survives auto-pruning, `Ctrl+S`). Both also protect from pruning. Tags were considered but deferred — these two axes plus type filters cover the same use cases visually without the schema/sync burden. |
...
| D13 | **Sync model split: text auto, files explicit** (from user direction) | `text/link/code/color/emoji` auto-sync via `CLIP_NEW` (toggle in settings, per direction). `image/file` NEVER cross the wire without an explicit user gesture — right-click "Send to phone", hover-overlay phone button, or drag onto the panel. This is the safe default for bandwidth + privacy. |
...
| User confuses Pin and Favorite (two stars-ish concepts) | Low | Distinct visuals: Pin = accent top stripe (no icon); Favorite = star icon (no stripe). Tooltips clarify on hover |
